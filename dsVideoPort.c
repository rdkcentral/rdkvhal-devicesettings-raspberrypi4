...
static void tvservice_hdcp_callback(void *callback_data, uint32_t reason,
	...
	switch (reason) {
		...
		default: {
			if (isBootup == true) {
				hal_warn(
				    "%s: At bootup HDCP status is "
				    "Authenticated for Rpi\n");
				_halhdcpcallback(
				    (int)(hdmiHandle->m_nativeHandle),
				    dsHDCP_STATUS_AUTHENTICATED);
				isBootup = false;
			}
			break;
...
dsError_t dsVideoPortInit() {
	...
	vc_tv_register_callback(&tvservice_hdcp_callback,
			...
			if (enabled) {
				...
				if (rc == -1) {
					hal_err(
					    ...
					    "rpiDisplayEnable.sh with enable=1 "
					    "rc=%d\n",
					    rc);
					return dsERR_GENERAL;
				}
				...
				if (rc == -1) {
					hal_err(
					    "Failed to run script "
					    "rpiDisplayEnable.sh with enable=0 "
					    "rc=%d \n",
					    rc);
					return dsERR_GENERAL;
				}
...
dsError_t dsEnableHDCP(intptr_t handle, bool contentProtect, char *hdcpKey,
	...
	if (!isValidVopHandle(handle) || NULL == hdcpKey ||
...
dsError_t dsGetResolution(intptr_t handle,
	...
	if (vc_tv_get_display_state(&tvstate) == 0) {
		...
		if (tvstate.state & VC_HDMI_ATTACHED) {
			...
			resolution_name =
			    dsVideoGetResolution(tvstate.display.hdmi.mode);
		} else {
	...
	// }
	if (resolution_name)
		strncpy(resolution->name, resolution_name,
		        strlen(resolution_name));
	return dsERR_NONE;
}
...
static uint32_t dsGetHdmiMode(dsVideoPortResolution_t *resolution) {
	...
	for (size_t i = 0; i < iCount; i++) {
		size_t length =
		    strlen(resolution->name) > strlen(resolutionMap[i].rdkRes)
		        ? strlen(resolution->name)
		        : strlen(resolutionMap[i].rdkRes);
	...
	if (!hdmi_mode) {
		hal_dbg(
		    "Given resolution not found, setting default Resolution "
		    "HDMI_CEA_720p60\n");
...
dsError_t dsSetResolution(intptr_t handle,
                          dsVideoPortResolution_t *resolution) {
	/* Auto Select uses 720p. Should be converted to dsVideoPortResolution_t
	 * = 720p in DS-VOPConfig, not here */
	hal_dbg("invoked.\n");
	VOPHandle_t *vopHandle = (VOPHandle_t *)handle;
	int res = 0;
	if (false == _bIsVideoPortInitialized) {
	...
	if (vopHandle->m_vType == dsVIDEOPORT_TYPE_HDMI) {
		...
		uint32_t hdmi_mode;
		hdmi_mode = dsGetHdmiMode(resolution);
		res = vc_tv_hdmi_power_on_explicit_new(
		    HDMI_MODE_HDMI, HDMI_RES_GROUP_CEA, hdmi_mode);
		if (res != 0) {
...
dsError_t dsSupportedTvResolutions(intptr_t handle, int *resolutions) {
	...
	if (resolutions != NULL && isValidVopHandle(handle) &&
	    vopHandle->m_vType == dsVIDEOPORT_TYPE_HDMI) {
		...
		num_of_modes = vc_tv_hdmi_get_supported_modes_new(
		    HDMI_RES_GROUP_CEA, modeSupported,
		    vcos_countof(modeSupported), &group, &mode);
		if (num_of_modes <= 0) {
			hal_err(
			    "vc_tv_hdmi_get_supported_modes_new() error, "
			    "Failed to get modes\n");
			return dsERR_GENERAL;
		}
		...
		hal_err(
		    "Error:Get supported resolution for TV on Non HDMI Port\n");
		return dsERR_INVALID_PARAM;
	}
...
dsError_t dsGetMatrixCoefficients(
    intptr_t handle, dsDisplayMatrixCoefficients_t *matrix_coefficients) {
	hal_dbg("invoked.\n");
	if (false == _bIsVideoPortInitialized) {
		return dsERR_NOT_INITIALIZED;
	}
...
dsError_t dsGetQuantizationRange(
    intptr_t handle, dsDisplayQuantizationRange_t *quantization_range) {
	hal_dbg("invoked.\n");
	if (false == _bIsVideoPortInitialized) {
		return dsERR_NOT_INITIALIZED;
	}
...
dsError_t dsGetCurrentOutputSettings(
    intptr_t handle, dsHDRStandard_t *video_eotf,
